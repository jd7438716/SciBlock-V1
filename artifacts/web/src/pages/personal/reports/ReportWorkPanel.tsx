 ...
 *
 * 职责（页面编排层 only）：
 *   - 从 props 接收报告数据，分发到子组件
 *   - 处理保存 / 提交 / 删除的异步操作
 *   - report = null 时渲染空状态占位
 *
 * 所有子功能已抽成独立组件：
 *   SelectedDatesSection    → components/reports/SelectedDatesSection.tsx
 *   LinkedExperimentsSection → components/reports/LinkedExperimentsSection.tsx
 *   DateEditModal / LinkEditModal 等内嵌于上述子组件内部
 *
 * 日期模型判断通过 isNewDateModel() selector 集中处理。
 */
...
import { SelectedDatesSection } from "@/components/reports/SelectedDatesSection";
import { LinkedExperimentsSection } from "@/components/reports/LinkedExperimentsSection";
...
export function ReportWorkPanel({
  ...
  const isEditable   = isReportEditable(report);
  const isNewModel   = isNewDateModel(report);
        ...
        {isEditable && (
          <div className="mb-6">
            ...
            <p className="text-xs text-gray-400 mt-1.5">
              将提交：
              <span className="font-medium text-gray-600">{fmtWeekLabel(report.weekStart)}</span>
